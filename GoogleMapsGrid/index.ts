...
export class GoogleMapsGrid implements ComponentFramework.StandardControl<IInputs, IOutputs> {
	...
	public updateView(context: ComponentFramework.Context<IInputs>): void
	{
        ...
        const latField: string = context.parameters.latFieldName.raw ? context.parameters.latFieldName.raw : "";
        const longField: string = context.parameters.longFieldName.raw ? context.parameters.longFieldName.raw : "";
        const nameField: string = context.parameters.primaryFieldName.raw ? context.parameters.primaryFieldName.raw : "";
        ...
        for (let i = 0; i < context.parameters.mapDataSet.paging.totalResultCount; i++) {
            ...
            const myLatLng = { lat: record.getValue(latField) as any, lng: record.getValue(longField) as any };
    ...
    */
    public buildInforWindow(recEntityType: string, recId: string, rec: DataSetInterfaces.EntityRecord, cols: DataSetInterfaces.Column[]): HTMLDivElement {
        ...
        const titleString = rec.getValue(primaryField).toString();
